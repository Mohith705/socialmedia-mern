    ...
    try {
        ...
        if (newVerifiedEmail) {
            transporter
                ...
                .catch((err) => {
                    console.log(err);
                    res.status(404).json({ message: "Please click on login or wait to redirect to login" });
                });
        }
    } catch (error) {
        console.log(error);
        res.status(404).json({ message: "Something went wrong here2" });
    }
};
    ...
    try {
        ...
        if (resetEmail) {
            transporter
                ...
                .catch((err) => {
                    console.log(err);
                    res.status(404).json({ message: "Something went wrong here3" });
                });
        }
    } catch (error) {
        console.log(error);
        res.status(404).json({ message: "Something went wrong here4" });
    }
};